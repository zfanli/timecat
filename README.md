...
我逐渐意识到，只有足够简单的记录方式，才能让我良好的保持时间记录的习惯。但是，足够简单也意味着精度的缺失。

我很幸运自己是一个开发者，所谓我打算尝试制作一个应用，探索精度和简单度中如何保持平衡，以此来满足自身的时间管理需求的同时，也是一种学习和投资。

足够简单的记录方式意味着友好的用户体验，但是由此导致数据精度缺失会限制后期分析的有效性，所以我得出的结论是，需要在要求用户录入的信息和数据结构上投入更多的思考才能保证两者之间的平衡。

用户体验和数据精度失衡或许会导致用户开心的记录了几个月的数据，结果发现这些数据不能作出任何有建设性的分析和警示，这对谁来说都是非常令人沮丧的。

数据才是最重要的。所以我设计这个应用时，我会投入更多的思考在如何保证数据尽量少的情况提供更多的信息，也就是如何让用户做更少的事情，让程序做更多的事。
